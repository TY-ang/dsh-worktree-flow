 ...
 *             sharedDocsPath?, repositories: {<component>: {label?, path, defaultBaseBranch?}}}.
 ...
 * @property {string} [sharedDocsPath] - optional project-wide untracked docs source.
 * @property {string} [projectInstructions] - live project guidance injected into every feature session.
 * @property {Record<string, RepoEntry>} repositories
 */
/** @typedef {{version?: number, label?: string, worktreeRoot?: string, defaultBaseBranch?: string, sharedDocsPath?: string, projectInstructions?: string, repositories?: Record<string, unknown>}} RawSetFile */

const CONTROL = /[\u0000-\u001f\u007f]/u;
const UNSAFE_INSTRUCTION_CONTROL = /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u;
const PROJECT_INSTRUCTIONS_MAX = 16 << 10;
...
export function normalizeProjectInstructions(value) {
	...
	if (typeof value !== "string") throw new WorktreeFlowError("BAD_CONFIG", "项目提示词必须是文本");
	const normalized = value.replace(/\r\n?/gu, "\n").trim();
	if (normalized === "") return undefined;
	if (Buffer.byteLength(normalized, "utf8") > PROJECT_INSTRUCTIONS_MAX || UNSAFE_INSTRUCTION_CONTROL.test(normalized)) {
		throw new WorktreeFlowError("BAD_CONFIG", "项目提示词无效或超过 16 KiB");
	}
	return normalized;
}
...
function normalizeSet(raw, name) {
	...
	const sharedDocsPath = /** @type {string | undefined} */ (expandHome(ne(raw?.sharedDocsPath, "sharedDocsPath", 4096)));
	if (sharedDocsPath !== undefined && !isPortableAbsolute(sharedDocsPath)) {
		throw new WorktreeFlowError("BAD_CONFIG", "sharedDocsPath 必须是绝对路径");
	}
	const projectInstructions = normalizeProjectInstructions(raw?.projectInstructions);
	return {
		name,
		label: ne(raw?.label, "label", 256),
		worktreeRoot,
		defaultBaseBranch,
		...(sharedDocsPath !== undefined ? { sharedDocsPath } : {}),
...
export function serializeSet(config) {
	...
	return {
		...
		...(config.sharedDocsPath !== undefined && config.sharedDocsPath !== "" ? { sharedDocsPath: config.sharedDocsPath } : {}),
		...(config.projectInstructions !== undefined && config.projectInstructions !== "" ? { projectInstructions: config.projectInstructions } : {}),
		repositories
	};
}
...
export async function readConfigTemplate() {
	...
	if (raw === undefined) return undefined;
	const worktreeRoot = typeof raw.worktreeRoot === "string" ? /** @type {string} */ (expandHome(cleanText(raw.worktreeRoot, "worktreeRoot"))) : "";
	if (worktreeRoot !== "" && !isPortableAbsolute(worktreeRoot)) throw new WorktreeFlowError("BAD_CONFIG", "worktreeRoot 必须是绝对路径");
	return {
		worktreeRoot,
...
export async function writeConfigTemplate(config) {
	...
	const worktreeRoot = config.worktreeRoot ? /** @type {string} */ (expandHome(cleanText(String(config.worktreeRoot), "worktreeRoot"))) : undefined;
	if (worktreeRoot !== undefined && !isPortableAbsolute(worktreeRoot)) throw new WorktreeFlowError("BAD_CONFIG", "worktreeRoot 必须是绝对路径");
	/** @type {RawSetFile} */
	const body = {
		version: 1,
