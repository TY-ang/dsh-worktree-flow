...
export function normalizeBranchName(branch) {
	if (typeof branch !== "string") {
		throw new WorktreeFlowError("BAD_BRANCH", `分支名无效：${String(branch)}`);
	}
	const trimmed = branch.trim();
	const parts = trimmed.split("/");
	const badPart = parts.some((part) => part === ""
		|| part.startsWith(".")
		|| part.endsWith(".")
		|| part.endsWith(".lock"));
	if (trimmed === ""
 ...
 * @property {string} [sessionInstructions] - feature-specific text injected on each session's first step.
