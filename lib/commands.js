	...
	switch (verb) {
		...
		case "sync": {
			...
			for (const action of result.actions) {
				const label = { registered: "已登记", retitled: "已登记(修正标题)", exists: "已登记", migrated: "清单已迁移", "skip-archived": "已归档跳过", unavailable: "登记不可用" }[action.action] ?? action.action;
		...
		case "open": {
			...
			if (feature.registration.state !== "registered") {
				await service.sync(setName, { signal });
			}
			return {
				kind: "success",
				text: [
		...
		case "finish": {
			...
			return {
				...
				text: [
					...
					"需要回看用 /worktree sync 恢复登记；要删目录用 /worktree finish <功能> --cleanup。"
				].join("\n")
			};
		}
