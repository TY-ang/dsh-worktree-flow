...
export function registerHttp(ctx, service) {
	...
	ctx.inject(["webServer"], (webCtx) => {
				...
				try {
					...
					if (req.method === "POST") {
						...
						const saved = await service.saveSetConfig({
							...
							sharedDocsPath: typeof config.sharedDocsPath === "string" && config.sharedDocsPath.trim() !== "" ? config.sharedDocsPath : undefined,
							projectInstructions: typeof config.projectInstructions === "string" && config.projectInstructions.trim() !== "" ? config.projectInstructions : undefined,
			...
			handler: async (req, res, signal) => {
				try {
					...
					const result = await service.createFeature(set, {
						...
						...(typeof intent.sessionInstructions === "string" ? { sessionInstructions: intent.sessionInstructions } : {}),
		...
		route({
			kind: "exact",
			path: "/worktree-flow/feature-instructions",
			methods: ["GET", "POST"],
			handler: async (req, res) => {
				try {
					if (req.method === "GET") {
						const result = await service.getFeatureInstructions(requireQuery(req, "set"), requireQuery(req, "feature"));
						sendJson(res, 200, { ok: true, result });
						return;
					}
					if (req.method === "POST") {
						const body = await readBody(req);
						if (typeof body.sessionInstructions !== "string") throw new WorktreeFlowError("BAD_INPUT", "sessionInstructions 必须是文本");
						const result = await service.saveFeatureInstructions(
							requireField(body, "set"),
							requireField(body, "feature"),
