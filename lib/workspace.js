...
const DUBIOUS_OWNERSHIP = /detected dubious ownership|dubious ownership/iu;

export function isDubiousOwnership(text) {
	return typeof text === "string" && DUBIOUS_OWNERSHIP.test(text);
}

function ownershipError(targetPath, detail) {
	return new WorktreeFlowError(
		"GIT_OWNERSHIP_MISMATCH",
		`Git 拒绝访问 worktree（目录所有者与当前用户不一致）：${targetPath}\n`
		+ "插件未修改 safe.directory 或目录所有者。请用与 IDEA 相同的普通 Windows 用户启动 DSH，修正该目录所有者或重新创建后再重试。"
		+ (detail === "" ? "" : `\nGit：${detail}`)
	);
}
...
async function mustGit(ctx, argv, cwd, signal, what) {
	...
	if (result.exitCode !== 0) {
		...
		if (isDubiousOwnership(detail)) throw ownershipError(cwd, detail);
		throw new WorktreeFlowError("GIT", `${what} 失败：${detail}`);
	}
...
/**
 * Resolve the base ref used for ahead/behind comparisons. Prefer the local
 * base branch's configured remote-tracking upstream, then origin/<base>, and
 * only fall back to the local branch. This keeps a stale local master/main
 * from hiding commits that are already visible in the fetched remote ref.
 */
export async function resolveComparisonBaseRef(ctx, repoPath, baseBranch, signal) {
	const upstream = await git(
		ctx,
		["for-each-ref", "--format=%(upstream)", `refs/heads/${baseBranch}`],
		repoPath,
		signal
	);
	if (upstream.exitCode === 0 && upstream.stdout.startsWith("refs/remotes/")) {
		const exists = await git(ctx, ["show-ref", "--verify", "--quiet", upstream.stdout], repoPath, signal);
		if (exists.exitCode === 0) return upstream.stdout;
	}
	const originRef = `refs/remotes/origin/${baseBranch}`;
	const origin = await git(ctx, ["show-ref", "--verify", "--quiet", originRef], repoPath, signal);
	if (origin.exitCode === 0) return originRef;
	return await branchExists(ctx, repoPath, baseBranch, signal) ? baseBranch : undefined;
}

/**
 * Verify that Git accepts the linked worktree under the same identity used by
 * the DSH host. This is intentionally read-only and never changes global Git
 * config, NTFS ownership, or safe.directory.
 */
export async function assertGitWorktreeUsable(ctx, targetPath, signal) {
	const result = await git(ctx, ["rev-parse", "--show-toplevel"], targetPath, signal);
	if (result.exitCode !== 0) {
		const detail = result.stderr || `exit ${result.exitCode}`;
		if (isDubiousOwnership(detail)) throw ownershipError(targetPath, detail);
		throw new WorktreeFlowError("GIT_WORKTREE_UNUSABLE", `Git 无法读取 worktree：${targetPath}\nGit：${detail}`);
	}
	if (canonical(result.stdout) !== canonical(targetPath)) {
		throw new WorktreeFlowError("GIT_WORKTREE_UNUSABLE", `Git worktree 根目录不一致：${targetPath} → ${result.stdout}`);
	}
...
export async function createLinkedWorktree(ctx, params) {
	...
	if (await pathExists(targetPath)) {
		...
		if (match !== undefined && match.branch === branch) {
			await assertGitWorktreeUsable(ctx, targetPath, signal);
 ...
 * @property {boolean} [ownershipMismatch] - Git rejected the repo as dubious ownership.
...
export async function componentGitStatus(ctx, params) {
	...
	if (branchResult.exitCode !== 0 && isDubiousOwnership(branchResult.stderr)) {
		return {
			present: true,
			ownershipMismatch: true,
			readError: ownershipError(dir, branchResult.stderr).message
		};
	}
	...
	if (statusResult.exitCode !== 0 && isDubiousOwnership(statusResult.stderr)) {
		return {
			present: true,
			branch,
			ownershipMismatch: true,
			readError: ownershipError(dir, statusResult.stderr).message
		};
	}
	...
	/** @type {number | undefined} */
	let behind;
	const baseRef = await resolveComparisonBaseRef(ctx, dir, baseBranch, signal);
	if (baseRef !== undefined) {
