...
export async function componentGitStatus(ctx, params) {
	...
	if (upstream.exitCode === 0) {
		...
		// No upstream configured: count commits unique to this branch vs base
		// that no remote has — in a remote-less repo this equals the ahead
		// count instead of "every commit ever".
		const local = await git(ctx, ["rev-list", "--count", `${baseRef}..HEAD`, "--not", "--remotes"], dir, signal);
		if (local.exitCode === 0) unpushed = Number.parseInt(local.stdout, 10);
	} else {
		const local = await git(ctx, ["rev-list", "--count", "HEAD", "--not", "--remotes"], dir, signal);
		if (local.exitCode === 0) unpushed = Number.parseInt(local.stdout, 10);
	}
