...
export class WorktreeFlowService {
	...
	async previewCreate(setName, intent, signal) {
		...
		const sessionInstructions = normalizeSessionInstructions(intent.sessionInstructions);
		const existingManifest = await readManifest(paths.featureRoot, { projectName: config.name, feature });
		const existingContext = existingManifest === undefined
			? undefined
			: await readFeatureContext(config.name, feature, paths.featureRoot);
		const effectiveSessionInstructions = sessionInstructions ?? existingContext?.sessionInstructions;
		const existingDocs = existingContext?.docsSnapshot;
		const targetPath = docsSnapshotPath(paths.featureRoot);
		const targetStat = await fs.promises.lstat(targetPath).catch((error) => {
			if (error.code === "ENOENT") return undefined;
			throw error;
		});
		const targetParentStat = targetStat === undefined ? undefined : await fs.promises.lstat(path.dirname(targetPath));
		const targetSafe = targetStat?.isDirectory() === true
			&& !targetStat.isSymbolicLink()
			&& targetParentStat?.isDirectory() === true
			&& !targetParentStat.isSymbolicLink()
			&& isPathWithin(paths.featureRoot, path.dirname(targetPath));
		const validatedExistingDocs = existingDocs !== undefined && targetSafe
			? await validateDocsSnapshot(existingContext, paths.featureRoot, signal)
			: undefined;
		let docsPlan;
		if (validatedExistingDocs !== undefined) {
			docsPlan = {
				sourcePath: existingDocs.sourcePath,
				targetPath,
				note: existingDocs.state === "copying"
					? "将完成已经复制成功的共享文档快照"
					: "将复用创建功能区时的共享文档快照",
				conflict: false,
				reuse: true,
				finalizePending: existingDocs.state === "copying"
			};
		} else {
			const sourcePath = existingDocs?.sourcePath ?? config.sharedDocsPath;
			if (sourcePath !== undefined) {
				await assertDocsDestinationParent(paths.featureRoot);
				const docsStat = await fs.promises.lstat(sourcePath).catch((error) => {
					if (error.code === "ENOENT") throw new WorktreeFlowError("NO_DOCS", `共享文档目录不存在：${sourcePath}`);
					throw error;
				});
				if (docsStat.isSymbolicLink()) throw new WorktreeFlowError("DOCS_SYMLINK", `共享文档源目录不允许符号链接或目录联接：${sourcePath}`);
				if (!docsStat.isDirectory()) throw new WorktreeFlowError("BAD_DOCS", `共享文档路径不是目录：${sourcePath}`);
				if (isPathWithin(sourcePath, targetPath) || isPathWithin(targetPath, sourcePath)) {
					throw new WorktreeFlowError("DOCS_OVERLAP", `共享文档源目录不能与功能区快照目录重叠：${sourcePath}`);
				}
				const conflict = targetStat !== undefined && (existingDocs === undefined || existingDocs.state !== "copying" || !targetSafe);
				docsPlan = {
					sourcePath,
					targetPath,
					note: existingDocs?.state === "copying" ? "将恢复未完成的共享文档快照" : "将复制项目共享文档快照",
					conflict,
					reuse: false
				};
		...
		return {
			...
			context: {
				hasInstructions: effectiveSessionInstructions !== undefined,
				instructionBytes: effectiveSessionInstructions === undefined ? 0 : Buffer.byteLength(effectiveSessionInstructions, "utf8"),
				docs: docsPlan
			},
			plan
		};
	...
	async assertFeatureWorktreesUsable(manifest, signal) {
		for (const component of Object.values(manifest.components)) {
			if (!fs.existsSync(component.path)) continue;
			await assertGitWorktreeUsable(this.ctx, component.path, signal);
		}
	}
	...
	async createFeatureUnlocked(preview, intent, options) {
		...
		if (preview.context.docs?.conflict === true) {
			conflicts.push({ component: "共享文档", note: `冲突：${preview.context.docs.targetPath} 已存在且不属于当前配置` });
		}
		...
		if (existing === undefined) {
			// Context is published only after the manifest. A record without its
			// manifest belongs to a removed/older generation and must not leak into
			// a same-name feature recreated at the same path.
			await deleteFeatureContext(preview.projectName, preview.feature);
		} else {
			existingContext = await readFeatureContext(preview.projectName, preview.feature, preview.featureRoot);
		}
		let docsSnapshot = existingContext?.docsSnapshot;
		if (docsSnapshot?.state === "copying" && preview.context.docs?.reuse !== true) {
			await removeDocsSnapshot(existingContext, preview.featureRoot);
			docsSnapshot = undefined;
		}
		let preparedDocs;
		if (preview.context.docs !== undefined && preview.context.docs.reuse !== true) {
			preparedDocs = await prepareDocsSnapshot(preview.context.docs.sourcePath, preview.featureRoot, options.signal);
		}
		await fs.promises.mkdir(preview.featureRoot, { recursive: true });
		const featureRootStat = await fs.promises.lstat(preview.featureRoot);
		if (!featureRootStat.isDirectory() || featureRootStat.isSymbolicLink()) {
			throw new WorktreeFlowError("BAD_LAYOUT", `功能根目录不是安全的普通目录：${preview.featureRoot}`);
		}
		...
		const incomingInstructions = normalizeSessionInstructions(intent.sessionInstructions);
		const sessionInstructions = incomingInstructions ?? existingContext?.sessionInstructions;
		const contextBase = {
			version: 1,
			projectName: preview.projectName,
			feature: preview.feature,
		...
		if (preview.context.docs !== undefined && preview.context.docs.reuse !== true) {
			const pendingDocs = {
				state: "copying",
				sourcePath: preview.context.docs.sourcePath,
				path: preview.context.docs.targetPath,
				createdAt: new Date().toISOString(),
				fileCount: preparedDocs.fileCount,
				bytes: preparedDocs.bytes
			};
			await writeFeatureContext({ ...contextBase, docsSnapshot: pendingDocs }, preview.featureRoot);
			docsSnapshot = await createDocsSnapshot(preview.context.docs.sourcePath, preview.featureRoot, {
				signal: options.signal,
				prepared: preparedDocs
			});
		}
		if (preview.context.docs?.reuse === true) {
			const validated = await validateDocsSnapshot(existingContext, preview.featureRoot, options.signal);
			docsSnapshot = validated === undefined ? undefined : { ...validated, state: "ready" };
		}
		if (sessionInstructions !== undefined || docsSnapshot !== undefined) {
			featureContext = await writeFeatureContext({
				...contextBase,
				...(docsSnapshot !== undefined ? { docsSnapshot } : {})
			}, preview.featureRoot);
		...
		for (const row of preview.plan) {
			...
			try {
				...
				const code = error instanceof WorktreeFlowError ? error.code : undefined;
				manifest.components[row.component] = { ...planned[row.component], state: "failed", error: message };
				results.push({ component: row.component, ok: false, error: message, ...(code !== undefined ? { code } : {}) });
				if (code === "GIT_OWNERSHIP_MISMATCH") {
		...
		return {
			...
			context: {
				...preview.context,
				docsSnapshot: featureContext?.docsSnapshot,
				hasInstructions: featureContext?.sessionInstructions !== undefined
			}
		};
	}
	 ...
	 */
	async getFeatureInstructions(setName, feature) {
		const record = await this.loadFeature(setName, feature);
		const context = await readFeatureContext(record.config.name, record.slug, record.paths.featureRoot);
		return { sessionInstructions: context?.sessionInstructions ?? "" };
	}

	async saveFeatureInstructions(setName, feature, value) {
		const initial = await this.loadFeature(setName, feature);
		return this.withFeatureLock(`${initial.config.name}/${initial.slug}`, async () => {
			const record = await this.loadFeature(setName, feature);
			const instructions = normalizeSessionInstructions(value);
			const context = await readFeatureContext(record.config.name, record.slug, record.paths.featureRoot);
			if (instructions === undefined && context?.docsSnapshot === undefined) {
				await deleteFeatureContext(record.config.name, record.slug);
				return { sessionInstructions: "" };
			}
			const next = await writeFeatureContext({
				version: 1,
				projectName: record.config.name,
				feature: record.slug,
				createdAt: context?.createdAt ?? record.loaded.manifest.createdAt,
				...(instructions !== undefined ? { sessionInstructions: instructions } : {}),
				...(context?.docsSnapshot !== undefined ? { docsSnapshot: context.docsSnapshot } : {})
			}, record.paths.featureRoot);
			return { sessionInstructions: next.sessionInstructions ?? "" };
		});
	}
	...
	async sync(setName, options = {}) {
		...
		for (const foundEntry of found) {
			...
			await this.withFeatureLock(`${config.name}/${foundEntry.manifest.feature}`, async () => {
				...
				if (register) {
					try {
						await this.assertFeatureWorktreesUsable(manifest, options.signal);
					} catch (error) {
						const code = error instanceof WorktreeFlowError ? error.code : "GIT_WORKTREE_UNUSABLE";
						actions.push({
							feature: manifest.feature,
							action: code === "GIT_OWNERSHIP_MISMATCH" ? "ownership-blocked" : "git-blocked",
	...
	async registerFeatureWorkspace(setName, feature) {
		...
		return this.withFeatureLock(`${initial.config.name}/${initial.slug}`, async () => {
			const record = await this.loadFeature(setName, feature);
			await this.assertFeatureWorktreesUsable(record.loaded.manifest);
	...
	async cleanupFeature(setName, feature, options = {}) {
		...
		return this.withFeatureLock(`${initial.config.name}/${initial.slug}`, async () => {
			...
			let cleanupContext;
			let forcedMetaRemoved = false;
			try {
				cleanupContext = await readFeatureContext(plan.manifest.projectName, plan.manifest.feature, plan.featureRoot);
			} catch (error) {
				if (options.force !== true) {
					throw new WorktreeFlowError("BLOCKED", `功能区可信上下文损坏；确认后使用强制清理：${error instanceof Error ? error.message : String(error)}`);
				}
				forcedMetaRemoved = await removeFeatureMeta(plan.featureRoot);
				await deleteFeatureContext(plan.manifest.projectName, plan.manifest.feature);
			}
			...
			let docsRemoved = forcedMetaRemoved;
			if (failed.length === 0) {
				try {
					docsRemoved = (await removeDocsSnapshot(cleanupContext, plan.featureRoot)) || docsRemoved;
				} catch (error) {
					failed.push(`共享文档快照: ${error instanceof Error ? error.message : String(error)}`);
				}
			}
			if (failed.length > 0) {
			...
			if (failed.length === 0) {
				...
				try {
					// The manifest is plugin-owned state. Remove it even when unrelated
					// user files keep the feature root alive, otherwise a later sync would
					// re-adopt a feature whose component worktrees were already deleted.
					await fs.promises.rm(path.join(plan.featureRoot, MANIFEST_NAME), { force: true });
					await fs.promises.rm(path.join(plan.featureRoot, LEGACY_MANIFEST_NAME), { force: true });
					const leftovers = await fs.promises.readdir(plan.featureRoot);
					if (leftovers.length === 0) {
						try {
							await fs.promises.rmdir(plan.featureRoot);
							rootRemoved = true;
						} catch (error) {
	...
	async validateConfig(setName, signal) {
		...
		let sharedDocs;
		if (config.sharedDocsPath !== undefined) {
			sharedDocs = { path: config.sharedDocsPath, exists: false, isDirectory: false };
			try {
				const stat = await fs.promises.lstat(config.sharedDocsPath);
				sharedDocs.exists = true;
				sharedDocs.isDirectory = stat.isDirectory() && !stat.isSymbolicLink();
				if (stat.isSymbolicLink()) sharedDocs.note = "不允许符号链接或目录联接";
				else if (!sharedDocs.isDirectory) sharedDocs.note = "路径存在但不是目录";
			} catch (error) {
				if (error.code === "ENOENT") sharedDocs.note = "目录不存在";
				else sharedDocs.note = error instanceof Error ? error.message : String(error);
			}
		}
		...
		const ok = problems.length === 0
			...
			&& (sharedDocs === undefined || sharedDocs.isDirectory)
			&& components.length > 0
			&& components.every((row) => row.issues.length === 0);
		return { ok, problems, worktreeRoot, sharedDocs, components };
	}
