...
export class WorktreeFlowService {
	...
	async previewCreate(setName, intent, signal) {
		...
		if (config.sharedDocsPath !== undefined) {
			const sourcePath = config.sharedDocsPath;
	...
	async createFeatureUnlocked(preview, intent, options) {
		...
		if (existing === undefined) {
			...
			// a same-name feature recreated at the same path. Remove plugin-owned
			// legacy docs/temp paths as part of that generation reset as well.
			await deleteFeatureContext(preview.projectName, preview.feature);
			await removeFeatureMeta(preview.featureRoot);
		} else {
			existingContext = await readFeatureContext(preview.projectName, preview.feature, preview.featureRoot);
		}
		// Keep legacy snapshot metadata only so cleanup can safely remove snapshots
		// created by older versions. New features use the live sharedDocsPath.
		const docsSnapshot = existingContext?.docsSnapshot;
	...
	async cleanupFeature(setName, feature, options = {}) {
		...
		return this.withFeatureLock(`${initial.config.name}/${initial.slug}`, async () => {
			...
			try {
				await readFeatureContext(plan.manifest.projectName, plan.manifest.feature, plan.featureRoot);
			} catch (error) {
			...
			if (failed.length === 0) {
				try {
					// Remove plugin-owned legacy snapshot/temp paths even when an old
					// context record was lost. removeFeatureMeta rejects linked roots.
					docsRemoved = (await removeFeatureMeta(plan.featureRoot)) || docsRemoved;
				} catch (error) {
					failed.push(`共享文档旧快照: ${error instanceof Error ? error.message : String(error)}`);
				}
			}
			if (failed.length > 0) {
