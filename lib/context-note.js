...
import { readFeatureContext, validateDocsSnapshot } from "./feature-context.js";
...
async function renderNote(manifest) {
	...
	const docsReady = featureContext?.docsSnapshot?.state === "ready"
		&& await validateDocsSnapshot(featureContext, manifest.root).then(() => true, () => false);
	const lines = [
	...
	if (docsReady && featureContext?.docsSnapshot !== undefined) {
		lines.push(`  项目共享文档快照：${featureContext.docsSnapshot.path}`);
		lines.push(`    这是创建功能区时复制的参考快照（${featureContext.docsSnapshot.fileCount} 个文件，请勿修改）；源目录后续变化不会自动同步。`);
	}
