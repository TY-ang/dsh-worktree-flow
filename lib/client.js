  ...
  factory: (require) => {
    ...
    function CreateWizard(props) {
      ...
      const intent = useMemo(() => ({
        ...
        registerComponents
      }), [picked, branchPreview, base, registerComponents]);
      ...
      const doPreview = useCallback(async () => {
        ...
        try {
          const data = await api("/create", { set: props.set, intent: { ...intent, dryRun: true } });
      ...
      const doCreate = useCallback(async () => {
        setBusy(true); setError(null);
        try {
          const data = await api("/create", { set: props.set, intent });
              ...
              children: [
                ...
                jsxs("div", {
                  ...
                  children: [
                    ...
                    jsx(Button, { primary: true, onClick: doCreate, disabled: busy || !ready || (preview !== null && Array.isArray(preview.plan) && preview.plan.some((row) => row.conflict)), children: busy ? "创建中…" : "确认创建" })
                  ]
                })
              ]
            })
    ...
    function CleanupModal(props) {
      ...
      useEffect(() => {
        let cancelled = false;
        api("/cleanup/plan", { set, feature: feature.feature })
      ...
      const run = useCallback(async () => {
        setBusy(true); setError(null);
        try {
          const data = await api("/cleanup", { set, feature: feature.feature, force });
          onDone(data.result);
        } catch (cause) { setError(cause.message); }
        finally { setBusy(false); }
        ...
        children: jsxs("div", {
          ...
          children: [
            ...
            jsxs("div", {
              ...
              children: [
                jsx(Button, { onClick: onClose, children: "取消" }),
                jsx(Button, { danger: true, onClick: run, disabled: busy || plan === null || (blocked && !force), children: busy ? "清理中…" : "确认清理" })
              ]
            })
          ]
        })
    ...
    function BranchTypesForm() {
      ...
      const addRow = () => {
        ...
        const prefix = newPrefix.trim().toLowerCase().replace(/\s+/g, "-").replace(/\/*$/u, "") + "/";
    ...
    function SetEditor(props) {
      ...
      useEffect(() => {
        let cancelled = false;
        setDraft(null); setSavedDraft(null); setValidation(null); setError(null);
        setProbeIssues({});
      ...
      const save = useCallback(async () => {
        ...
        setBusy(true); setError(null);
        try {
          await api("/config", { set, config: draft });
          setSavedDraft(draft);
          setSavedAt(new Date());
          onSaved();
    ...
    function WorktreeFlowSection(props) {
      ...
      useEffect(() => {
        ...
        api(`/features?set=${encodeURIComponent(current)}`)
          .then((data) => { if (!cancelled) setFeatures(data.features); })
    ...
    function fetchLocate(cwd, detail, force) {
      ...
      if (hit !== undefined && Date.now() - hit.at < LOCATE_TTL_MS) return Promise.resolve(hit.data);
      let pending = locateInflight.get(key);
      if (pending === undefined) {
        pending = api(`/locate?cwd=${encodeURIComponent(cwd)}${detail === true ? "&detail=1" : ""}`)
          .then((data) => { locateCache.set(key, { at: Date.now(), data }); locateInflight.delete(key); return data; })
          .catch((error) => { locateInflight.delete(key); throw error; });
        locateInflight.set(key, pending);
      }
      return pending;
    }
    ...
    function FeatureBadgeAction(kit) {
      ...
      const loadDetail = useCallback(() => {
        ...
        fetchLocate(cwd, true, true)
          .then((data) => setDetail(data.found === true ? data : null))
          .catch(() => { /* 保留旧详情 */ });
      }, [cwd]);
      useEffect(() => { setDetail(null); }, [cwd]);
      useEffect(() => {
        if (!open) return undefined;
        const onDocDown = (e) => { if (rootRef.current !== null && !rootRef.current.contains(e.target)) setOpen(false); };
        const onKey = (e) => { if (e.key === "Escape") setOpen(false); };
      ...
      return jsxs("div", {
        ...
        children: [
          jsxs("button", {
            ...
            ref: triggerRef,
            title: "功能工作区",
            onClick: () => { const next = !open; setOpen(next); if (next) loadDetail(); },
            style: {
            ...
            children: [
              ...
              jsxs("div", {
                ...
                children: [
                  ...
                  jsx(Button, { small: true, onClick: loadDetail, children: "刷新" })
                ]
              })
            ]
          })
