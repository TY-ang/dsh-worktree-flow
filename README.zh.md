...
- **配置**：仓库组列表 + 编辑器。点「+ 新建仓库组」起名（名字会拼进功能工作区目录名，创建后不可改），初始内容按「新仓库组模板」预填一次；然后绑定组件：组件行默认只显示「名字 → 路径」一行，点开才编辑——路径点「选择…」挑选后立即探测（是不是 git 仓库、基准分支是什么），选到非 git 目录时行内给「初始化 git 仓库」按钮；保存后自动行内验证（仓库存在/git 仓库/基准分支/根目录可写）。可选的「共享 docs 源目录」用于仓库组级、未跟踪 Git 的项目文档；所有功能区直接读取同一个原始目录
...
- **共享 docs 原始目录**：如果仓库组配置了源目录，功能区会话直接读取该目录，因此主目录修改立即可见，不再生成每功能区快照。普通工作区沙箱不能写根目录之外；插件提供 `worktree_docs_write` / `worktree_docs_edit`，仅允许当前仓库组的功能区会话按相对路径修改该共享目录，并拒绝绝对路径、越界路径及符号链接/目录联接
...
| 项目共享 docs | 仓库组配置的 `sharedDocsPath` 原始目录（所有功能区共用） |
